function qMRFit_BIDS_MATLABonly_singleSub(inputDir)
...
BIDS_subjects = intersect(cellstr(config_file.Subject),BIDS_subjects);

for sub_iter = 1:length(BIDS_subjects)
    
    if length(BIDS_subjects) > 1
        disp('This script is designed to process data at a single-subject level')
        break
    end
    
    BIDS_types = bids.query(BIDS,'types','sub',BIDS_subjects(sub_iter));
    BIDS_types = intersect(cellstr(config_file.Modality), BIDS_types);
